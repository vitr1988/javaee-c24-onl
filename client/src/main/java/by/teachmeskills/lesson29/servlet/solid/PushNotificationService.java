package by.teachmeskills.lesson29.servlet.solid;

import by.teachmeskills.lesson29.servlet.solid.domain.Order;
import by.teachmeskills.lesson29.servlet.solid.exception.NotImplementedYetException;

public class PushNotificationService implements AbstractPushNotificationService {

    @Override
    public void sendNotification(Order order) {
        throw new NotImplementedYetException(PushNotificationService.class.getSimpleName());
    }

    @Override
    public void sendPush(String phone) {
        //TODO:
    }
}
